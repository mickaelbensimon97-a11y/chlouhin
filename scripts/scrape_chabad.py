...
Intégration des données dans Supabase
"""
...
class ChabadScraper:
    ...
    def extract_center_data(self, center: Dict) -> Dict:
        ...
        return {
            ...
            "latitude": latitude,
            "longitude": longitude,
            "directors": json.dumps([]),  # Non disponible dans l'API
            "created_at": datetime.now(timezone.utc).isoformat(),
            "updated_at": datetime.now(timezone.utc).isoformat()
        }

    def insert_into_supabase(self, center_data: Dict) -> bool:
        """
        Insère un centre dans Supabase
        """
        try:
            url = f"{self.supabase_url}/rest/v1/beth_habad"
            response = requests.post(url, json=center_data, headers=self.headers)
            response.raise_for_status()
            self.success_count += 1
            return True
        except requests.exceptions.RequestException as e:
            error_msg = f"Erreur lors de l'insertion dans Supabase: {str(e)}"
            print(f"❌ {error_msg}")
            self.errors.append(error_msg)
            return False
            ...
            for center in centers:
                try:
                    ...
                    if center_data["name"]:
                        ...
                        self.insert_into_supabase(center_data)
                        total_processed += 1
    ...
    def print_summary(self):
        ...
        print(f"✅ Centres insérés dans Supabase: {self.success_count}")
